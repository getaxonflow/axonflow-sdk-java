...
- **`AxonFlow.AUTHZEN_PATH` and `AUTHZEN_PROFILE_HEADER` are generated from the
  surface artifact, not written here (axonflow-enterprise#3603 follow-up).** The
  route (`/api/v1/access/evaluation`) and the negotiation header
  ...
  (`route`, `profile_header`); the generator requires them and emits
  `AuthZENContract.PATH` and `AuthZENContract.PROFILE_HEADER`; the two
  `AxonFlow` constants now read those. The vendored artifact is refreshed to the
  publishing platform commit. No wire change.
