...
# scripts/snapshot_openapi_schemas.py opens every file it writes with this
# line, and its second line names the platform commit the file came from.
SNAPSHOT_HEADER_FIRST_LINE = (
    "# GENERATED by scripts/snapshot_openapi_schemas.py - do not edit."
)
_SNAPSHOT_SOURCE_RE = re.compile(
    r"^# Source: \S+ at platform commit ([0-9a-f]{40})$"
)


def snapshot_source_commit(spec_dir: Path) -> str | None:
    """Return the platform commit a generated snapshot was derived from.

    None when no *.yaml in spec_dir carries the generated header (a
    checkout of the platform's docs/api). Exits with an error when only
    some files carry it, when a header does not name a full commit, or
    when the files name different commits: a partly regenerated
    snapshot has no single revision to pin.
    """
    commits: dict[str, str] = {}
    plain: list[str] = []
    for spec_file in sorted(spec_dir.glob("*.yaml")):
        head = spec_file.read_text(encoding="utf-8").splitlines()[:2]
        if not head or head[0] != SNAPSHOT_HEADER_FIRST_LINE:
            plain.append(spec_file.name)
            continue
        match = _SNAPSHOT_SOURCE_RE.match(head[1]) if len(head) > 1 else None
        if match is None:
            raise SystemExit(
                f"❌ {spec_file}: the generated header does not name the full "
                "40-character platform commit the file was derived from."
            )
        commits[spec_file.name] = match.group(1)
    if not commits:
        return None
    if plain:
        raise SystemExit(
            f"❌ {spec_dir} mixes generated snapshot files with plain specs "
            f"({', '.join(plain)}); regenerate the whole snapshot."
        )
    distinct = sorted(set(commits.values()))
    if len(distinct) > 1:
        named = ", ".join(f"{name} at {sha[:9]}" for name, sha in sorted(commits.items()))
        raise SystemExit(
            f"❌ {spec_dir} holds snapshot files from different platform "
            f"commits ({named}); regenerate the whole snapshot at one commit."
        )
    return distinct[0]
...
def load_baseline() -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            ...
            f"   Regenerate via:\n"
            f"     python3 scripts/wire_shape/refresh.py tests/fixtures/openapi"
        ) from None
    base = empty_baseline()
    base.update(parsed)
    return base
