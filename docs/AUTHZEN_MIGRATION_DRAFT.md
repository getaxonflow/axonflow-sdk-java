...
The short version: **write new integrations against `evaluate` / `evaluateAll`. Leave working integrations alone.**

## Timeline

| release | the AuthZEN surface | the legacy decision surface |
|---|---|---|
| v10.3.0 (this) | New. Available. Recommended for new integrations. An **adapter** over the same evaluation `POST /api/v1/decide` runs. | Fully supported. Not deprecated. No warnings. Unchanged, byte for byte. |
...
| `query` | `context.args.query` | `AuthZENEvaluation.query(...)` |
...
| `context` (allowlisted headers) | `context.correlation.<key>` | `AuthZENEvaluation.correlation(...)`; the deployment's allowlist and its key cap still apply, and a key it does not record is refused rather than dropped |
| `verdict: "allow" / "deny" / "needs_approval"` | `decision` + `context.state` (`ALLOW` / `DENY` / `CHALLENGE` / `ERROR`) | `AuthZENDecision.isAllowed()` requires both |
