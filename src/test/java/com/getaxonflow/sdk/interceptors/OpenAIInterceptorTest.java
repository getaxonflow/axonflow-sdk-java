...
class OpenAIInterceptorTest {
  ...
  class TypeTests {
    ...
    void testChatCompletionRequestRequiresModel() {
      assertThatThrownBy(() -> ChatCompletionRequest.builder().addUserMessage("Test").build())
              ...
              .choices(
                  List.of(
                      new ChatCompletionResponse.Choice(
                          0, ChatMessage.assistant("Test response"), "stop")))
      ...
      ChatCompletionResponse response =
          ChatCompletionResponse.builder()
              .choices(
                  List.of(
                      new ChatCompletionResponse.Choice(
                          0, ChatMessage.assistant(longContent), "stop")))
              .build();
  ...
  class IntegrationTests {
    ...
    void testWrapAllowedRequest() {
      ...
      Function<ChatCompletionRequest, ChatCompletionResponse> mockCall =
          request ->
              ChatCompletionResponse.builder()
                  .id("chatcmpl-123")
                  .model("gpt-4")
                  .choices(
                      List.of(
                          new ChatCompletionResponse.Choice(
                              0, ChatMessage.assistant("Hello! How can I help you?"), "stop")))
          ...
          post(urlEqualTo("/api/request"))
              .willReturn(
                  aResponse()
                      ...
                      .withHeader("Content-Type", "application/json")
                      .withBody(
                          "{\"success\":false,\"blocked\":true,\"block_reason\":\"Request blocked by policy: no-pii\"}")));
              ...
              CompletableFuture.completedFuture(
                  ChatCompletionResponse.builder()
                      ...
                      .choices(
                          List.of(
                              new ChatCompletionResponse.Choice(
                                  0, ChatMessage.assistant("Async response"), "stop")))
                      .usage(ChatCompletionResponse.Usage.of(5, 15))
                      .build());

      // Create request
      ChatCompletionRequest request =
          ChatCompletionRequest.builder().model("gpt-4").addUserMessage("Async test").build();

      // Execute wrapped async call
      ChatCompletionResponse response = interceptor.wrapAsync(mockCall).apply(request).get();
    ...
    void testWrapAsyncBlockedRequest() {
          ...
          post(urlEqualTo("/api/request"))
              .willReturn(
                  aResponse()
                      ...
                      .withHeader("Content-Type", "application/json")
                      .withBody(
                          "{\"success\":false,\"blocked\":true,\"block_reason\":\"Content policy violation\"}")));
      ...
      // Create request
      ChatCompletionRequest request =
          ChatCompletionRequest.builder().model("gpt-4").addUserMessage("Blocked content").build();
    ...
    void testStaticWrapperMethod() {
      ...
      // Create mock OpenAI call
      Function<ChatCompletionRequest, ChatCompletionResponse> mockCall =
          request -> ChatCompletionResponse.builder().id("chatcmpl-789").model("gpt-4").build();

      // Use static wrapper
      ChatCompletionRequest request =
          ChatCompletionRequest.builder().model("gpt-4").addUserMessage("Static test").build();

      ChatCompletionResponse response =
          OpenAIInterceptor.wrapChatCompletion(axonflow, "user-token", mockCall).apply(request);
