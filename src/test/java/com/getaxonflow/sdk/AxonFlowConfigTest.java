...
class AxonFlowConfigTest {
  ...
  void shouldCreateMinimalLocalhostConfig() {
    AxonFlowConfig config = AxonFlowConfig.builder().endpoint("http://localhost:8080").build();
  ...
  void shouldCreateFullConfig() {
    RetryConfig retryConfig =
        RetryConfig.builder().maxAttempts(5).initialDelay(Duration.ofMillis(500)).build();

    CacheConfig cacheConfig = CacheConfig.builder().ttl(Duration.ofMinutes(5)).maxSize(500).build();
  ...
  void shouldDetectLocalhost(String url) {
    AxonFlowConfig config = AxonFlowConfig.builder().endpoint(url).build();
  ...
  void shouldAllowNonLocalhostWithoutCredentials() {
    ...
    AxonFlowConfig config = AxonFlowConfig.builder().endpoint("https://api.example.com").build();
  ...
  void shouldUseDefaultRetryConfig() {
    ...
    assertThat(config.getRetryConfig().getMaxAttempts())
        .isEqualTo(RetryConfig.DEFAULT_MAX_ATTEMPTS);
  }
  ...
  void shouldImplementEqualsAndHashCode() {
    AxonFlowConfig config1 =
        AxonFlowConfig.builder().endpoint("http://localhost:8080").clientId("test").build();

    AxonFlowConfig config2 =
        AxonFlowConfig.builder().endpoint("http://localhost:8080").clientId("test").build();

    AxonFlowConfig config3 = AxonFlowConfig.builder().endpoint("http://localhost:8081").build();
