...
class PlanTypesTest {
  ...
  void planRequestShouldBuildWithRequired() {
    PlanRequest request = PlanRequest.builder().objective("Research AI governance").build();
  ...
  void planResponseShouldDetectFailed() throws Exception {
    String json =
        "{" + "\"plan_id\": \"plan_abc123\"," + "\"steps\": []," + "\"status\": \"failed\"" + "}";
  ...
  void planResponseShouldHandleEmptySteps() throws Exception {
    String json = "{" + "\"plan_id\": \"plan_abc123\"," + "\"status\": \"pending\"" + "}";
