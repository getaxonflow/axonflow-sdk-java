...
class ClientRequestTest {
  ...
  void shouldBuildWithRequiredFields() {
    ClientRequest request = ClientRequest.builder().query("What is the weather?").build();
  ...
  void shouldAddContextEntries() {
    ...
    assertThat(request.getContext()).containsEntry("key1", "value1").containsEntry("key2", 42);
  }
  ...
  void shouldReturnImmutableContext() {
    ClientRequest request =
        ClientRequest.builder().query("test").context(Map.of("key", "value")).build();
  ...
  void shouldImplementEqualsAndHashCode() {
    ClientRequest request1 = ClientRequest.builder().query("test").userToken("user-123").build();

    ClientRequest request2 = ClientRequest.builder().query("test").userToken("user-123").build();

    ClientRequest request3 =
        ClientRequest.builder().query("different").userToken("user-123").build();
