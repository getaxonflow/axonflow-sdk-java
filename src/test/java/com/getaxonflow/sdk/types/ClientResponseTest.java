...
class ClientResponseTest {
  ...
  void shouldDeserializeSuccessResponse() throws Exception {
    ...
    assertThat(response.getPolicyInfo().getPoliciesEvaluated())
        .containsExactly("policy1", "policy2");
  ...
  void shouldDeserializeBlockedResponse() throws Exception {
    ...
    assertThat(response.getBlockReason())
        .isEqualTo("Request blocked by policy: sql_injection_detection");
  ...
  void shouldExtractPolicyName(String blockReason, String expectedPolicy) throws Exception {
    String json =
        String.format(
            "{" + "\"success\": false," + "\"blocked\": true," + "\"block_reason\": \"%s\"" + "}",
            blockReason);
  ...
  void shouldHandleNullBlockReason() throws Exception {
    String json = "{" + "\"success\": true," + "\"blocked\": false" + "}";
  ...
  void shouldImplementEqualsAndHashCode() throws Exception {
    String json = "{" + "\"success\": true," + "\"blocked\": false," + "\"data\": \"test\"" + "}";
