...
class ExceptionsTest {
  ...
  void policyViolationExceptionExtractsPolicyName() {
    PolicyViolationException ex =
        new PolicyViolationException("Request blocked by policy: sql_injection_detection");
  ...
  void policyViolationExceptionWithExplicitPolicy() {
    PolicyViolationException ex =
        new PolicyViolationException(
            "PII detected in request", "pii_detection", List.of("pii_detection", "rate_limit"));
  ...
  void connectorException() {
    ConnectorException ex =
        new ConnectorException("Connector query failed", "salesforce", "getAccounts");
  ...
  void planExecutionException() {
    PlanExecutionException ex = new PlanExecutionException("Step failed", "plan_123", "step_002");
