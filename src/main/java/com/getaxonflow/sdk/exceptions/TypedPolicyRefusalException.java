 ...
 * (402, with {@link #getCode()} naming the limit) or {@code artifact_cap} (429). {@link
 * #getFindings()} holds the declared findings a refused publication or document carries, and {@link
 * #getRetryAfter()} the seconds from {@code Retry-After} when the refusal is retryable. The message
 * is the platform's own explanation. A 401 is an {@link AuthenticationException}, as on every other
 * route.
 */
