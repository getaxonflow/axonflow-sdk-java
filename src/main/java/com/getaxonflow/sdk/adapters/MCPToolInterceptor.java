 ...
 *
 * <p>Wraps each MCP tool invocation with pre-call input validation and
...
public final class MCPToolInterceptor {
    ...
    MCPToolInterceptor(AxonFlow client, Function<MCPToolRequest, String> connectorTypeFn, String operation) {
        this.client = client;
     ...
     * <p>Flow:
     * <ol>
     *   <li>Derive connector type from the request</li>
     *   <li>Build a statement string and call {@code mcpCheckInput}</li>
     *   <li>If blocked, throw {@link PolicyViolationException}</li>
     *   <li>Execute the handler</li>
     *   <li>Call {@code mcpCheckOutput} with the result</li>
     *   <li>If blocked, throw {@link PolicyViolationException}</li>
     *   <li>If redacted data is present, return it instead of the raw result</li>
     * </ol>
     *
