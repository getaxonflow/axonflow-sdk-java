...
1. Nothing is active yet: `active()` is empty from the platform's `404 nothing_active`.
2. `edition()` reports the deployment's boundary and names its vocabulary: `getCatalogDigest()`, a `getRegistryVersion()` above 0, and `getCatalogFixture()` false. `system()` returns the shipped controls with their digest, and reads each control's `getName()` and which are `getMandatory()`.
3. The document the platform's own route test proves publishable validates clean, publishes to a digest, and activates. The publication reports the organization template controls the document omits, as `getTemplateOmissions()`: all of them, since the fixture is minimal. The activation reports the same omissions, beside the activation record.
4. `active()` returns that document as the exact signed source, carrying the policies that were published. The platform overwrites the author: the document deliberately names `someone-else`, and the platform signs the caller the agent resolved (on Community, `Client` in the `axonflow-api-credential` realm, named by the client id).
...
A refusal's `getPolicy()` (a tier refusal) and the `getTemplateOmissionsUnavailable()` reasons are proven by the unit tests only: a Community stack crosses no tier ceiling, and reads its template.
