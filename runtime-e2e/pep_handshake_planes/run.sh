#!/usr/bin/env bash
# Runs the PEP handshake runtime leg against a real AxonFlow agent.
# Usage: AXONFLOW_AGENT_URL=http://localhost:8080 AXONFLOW_CLIENT_ID=... \
#        AXONFLOW_CLIENT_SECRET=... AXONFLOW_USER_TOKEN=... \
...
mvn -q -DskipTests dependency:build-classpath -Dmdep.outputFile=target/pep-handshake-cp.txt
...
CP="${SDK_JAR}:${SLF4J_SIMPLE}:$(cat target/pep-handshake-cp.txt)"

AXONFLOW_TELEMETRY=off java -cp "$CP" runtime-e2e/pep_handshake_planes/PEPHandshakePlanesTest.java
