...
Real-stack proof that the two v11 examples run as the README says. `run.sh` builds the SDK from this tree, compiles `examples/pep-handshake` and `examples/typed-policies` against it into a temporary directory, and runs them against a live Community agent in the README's order: the handshake example first. Every run starts from that directory, outside the tree, with no body file: `typed-policies` finds its default document on its classpath, as its own build puts it there. Telemetry is off. Nothing is mocked.
...
- **That the handshake reaches the wire.** A fresh Community stack allows the first decide with or without a declaration, and the `/pep_id` refusal happens in the client.
...
It leaves `AXONFLOW_CLIENT_ID` and `AXONFLOW_CLIENT_SECRET` unset, so the examples present no credentials, and on Community the organization is the deployment's (`ORG_ID`). It ignores any `AXONFLOW_TYPED_POLICY_PUBLISH` or `AXONFLOW_TYPED_POLICY_BODY` it was started with: each run sets its own. It exits 0 when every assertion passes, 1 when one fails, and 2 when the precondition does not hold (the agent does not answer, or a typed document is already active). It changes the organization's active policy.
